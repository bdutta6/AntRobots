...
void HMC5883L:: setup(){
  ...
  error = SetScale(comp_scale); // Set the scale of the compass.
  if(error != 0){// If there is an error, print it out.
    Serial.println(GetErrorText(error));
	error = SetScale(comp_scale);
  }
...
bool HMC5883L :: Detected(){
	int max_reading = 0;
  for (int i = 0; i < 150; i++){
	MagnetometerRaw raw = ReadRawAxis();
	MagnetometerScaled scaled = ReadScaledAxis();
	int max_axis = max(max(1023/raw.XAxis,1023/raw.YAxis),1023/raw.ZAxis);
	max_reading = max(max_reading, max_axis);
	}
	if (max_reading == 1023){
		return true;
	}
